...
const rooms=(process.env.ROOMS||'1,2,3,4,5,6,7').split(',').map(Number);
assert.ok(rooms.every(n=>Number.isInteger(n)&&n>=1&&n<=7));
...
try{
 for(const room of rooms){
  ...
  assert.equal(info.features.defaultEdition,'foundation');assert.deepEqual(info.features.foundation.rooms,[1,2,3,4,5,6,7]);
  assert.equal(await page.evaluate(()=>window.__NESI_DEMO_GAME__.chamberEdition),'foundation');
  assert.deepEqual(await page.$$eval('#level-select option',a=>a.map(e=>Number(e.value))),[0,1,2,3,4,5,6]);
  assert.equal(await page.$eval('#level-select',e=>Number(e.value)),room-1);
  assert.match(await page.$eval('#campaign-count',e=>e.textContent),/7 испытаний/);
  ...
  await page.locator('#play-button').click();await page.waitForFunction(()=>window.__NESI_DEMO_GAME__?.state==='playing');
  ...
  const cases=process.env.ONLY_NORMAL==='1'?[{}]:[{}, {alternate:true}, ...([1,5,6,7].includes(room)?[{recover:true}]:[])];
  try{
   for(const [index,options]of cases.entries()){
    await page.evaluate(({room,record})=>{
     ...
     g.updateVisuals=(...a)=>{update(...a);frame++;const p=g.playerPosition,l=g.firstLevel;
      const active=room===1?g.teleportCount>0:room===2?(g.heldCube&&p.x>-10&&p.x<10):room===3?(l.car&&!l.car.at(l.car.target)&&p.y>4):room===4?(!g.playerGrounded&&g.teleportCount>0):room===5?(l.cabin.position.y>.4&&l.cabin.position.y<5.8):room===6?(l.balance.height>.3&&l.balance.height<7.7):(!l.stage.at(l.stage.target)||g.teleportCount>0);
   ...
   await page.waitForFunction(next=>window.__NESI_DEMO_GAME__?.state==='playing'&&window.__NESI_DEMO_GAME__.levelIndex===next,{},room===7?0:room);
   if(room===7)assert.match(await page.$eval('#win-screen .muted',e=>e.textContent),/Две главы/);
   await page.evaluate(()=>{const g=window.__NESI_DEMO_GAME__;g.renderer.setAnimationLoop(null);g.render();});
   await page.screenshot({path:path.join(out,`${room}-next.png`)});
   fs.writeFileSync(path.join(out,`${room}-ui.json`),JSON.stringify({source:info.commit,defaultEntry:true,saveIsolated:true,transition:{from:room,to:room===7?1:room+1},errors},null,2));
