...
assert.ok(rooms.every(n=>Number.isInteger(n)&&n>=1&&n<=30));
...
try{
 for(const room of rooms){
  ...
  // The opening uses the unqualified public entry; later bookmarks remain archive links.
  const url=new URL(base);url.search=`?debug=1${room===1?'':'&level='+room}${room>5?'&edition=foundation':''}`;
  ...
  assert.equal(info.features.defaultEdition,'foundation');assert.deepEqual(info.features.foundation.rooms,Array.from({length:30},(_,i)=>i+1));
  assert.equal(await page.evaluate(()=>window.__NESI_DEMO_GAME__.chamberEdition),'foundation');
  assert.deepEqual(await page.$$eval('#level-select option',a=>a.map(e=>Number(e.value))),Array.from({length:30},(_,i)=>i));
  assert.equal(await page.$eval('#level-select',e=>Number(e.value)),room-1);
  assert.match(await page.$eval('#campaign-count',e=>e.textContent),/30 испытаний/);
  ...
  const cases=process.env.ONLY_NORMAL==='1'||room>5?[{}]:[{}, {alternate:true}, ...([1,5].includes(room)?[{recover:true}]:[])];
  try{
   ...
   // The real victory control advances room 5 into room 6 and wraps only after room 30.
   await page.waitForFunction(()=>!document.pointerLockElement&&getComputedStyle(document.querySelector('#win-screen')).opacity==='1');
   if(room===30)assert.match(await page.$eval('#win-screen .muted',e=>e.textContent),/30 испытаний кампании/);
   await page.locator('#play-again-button').click();
   await page.waitForFunction(next=>window.__NESI_DEMO_GAME__?.state==='playing'&&window.__NESI_DEMO_GAME__.levelIndex===next,{},room===30?0:room);
   await page.evaluate(()=>{const g=window.__NESI_DEMO_GAME__;g.renderer.setAnimationLoop(null);g.render();});
   await page.screenshot({path:path.join(out,`${room}-next.png`)});
   fs.writeFileSync(path.join(out,`${room}-ui.json`),JSON.stringify({source:info.commit,defaultEntry:room<=5,saveIsolated:true,transition:{from:room,to:room===30?1:room+1},errors},null,2));
