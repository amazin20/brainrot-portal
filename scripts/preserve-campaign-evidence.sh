...
REVIEW_REF="evidence/campaign26-${REVIEW_KIND}-${GITHUB_SHA:0:12}-${GITHUB_RUN_ID}-${GITHUB_RUN_ATTEMPT}"
REVIEW_WORK="$(mktemp -d /tmp/campaign26-evidence.XXXXXX)"
...
git -C "$REVIEW_WORK" -c user.name='github-actions[bot]' -c user.email='41898282+github-actions[bot]@users.noreply.github.com' commit -m "Campaign26 ${REVIEW_KIND} evidence for $GITHUB_SHA"
