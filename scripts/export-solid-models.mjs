...
const out = path.resolve(repo, process.argv[2] || 'qa/solid-models');
await fs.mkdir(out, { recursive: true });
const assets = [
 ['botanical-planter',()=>createPlanter('garden'),{},'Y-up, base at 0; physical crown and ceramic bowl.'],
 ['conveyor-gearmotor',()=>createDrive(),{},'Horizontal spindle along X; bolted saddle underneath.'],
 ['ribbed-canopy',()=>createCanopy(),{},'Open walkable arch, four posts; CLOSED thick roof.'],
 ['flight-collar',()=>createGuideRing(),{},'Local XY open aperture; separate collision arc segments.'],
 ['field-resonator',()=>createCrystal(),{},'Y-up, closed crystal and cage on a solid plinth.'],
 ['tidal-pontoon',()=>createPontoon(8.8,7.8),{},'Attach beneath actual tidal slider deck, Y-up.'],
 ['solar-observatory',()=>createSolarObservatory(),{},'Central pedestal, cantilever spars and folded solar fins.'],
];
...
const sourcePath = path.join(repo, 'src/game/LabSolidModels.js');
...
for (const [name, builder, options, contract] of assets) {
  ...
  const report = { name, file: filename, bytes: bytes.byteLength, sha256: crypto.createHash('sha256').update(bytes).digest('hex'),
    source: 'src/game/LabSolidModels.js', sourceSha256, reproducibleCommand: 'node scripts/export-solid-models.mjs',
