...
const tag=process.env.P04_TAG??'after',out=process.env.EVIDENCE_OUT??'smoke-artifacts/p04b';
...
try{
 ...
 report.summary=[3,4].map(event=>{
  ...
  const postSteps=a.slice(1).map((s,i)=>({time:s.after,distance:Math.hypot(...s.camera.map((v,j)=>v-a[i].camera[j]))}));
  return{event,held:a[0]?.held,postFrames:a.length,postMinRetained:Math.min(...a.map(s=>s.skin.retained)),
   maxLateCameraStep:Math.max(0,...postSteps.filter(s=>s.time>=.4).map(s=>s.distance)),earlyFrames:early.length,earlyLost:early.filter(s=>s.skin.outside>0).length,
