...
const report=await runV8Journey(g,{scenario:d=>runRampExploration(d,mark=>{
 samples++;let error=0;
 for(const y of [0,1.2,2.5]){const v=g.playerPosition.clone().add(new THREE.Vector3(0,y,0)).project(g.camera);error=Math.max(error,Math.abs(v.x)-1,Math.abs(v.y)-1,v.z>1?1:0);}
 if(error>0){outside++;first??={...mark,camera:g.camera.position.toArray(),error};}worst=Math.max(worst,error);
})});
