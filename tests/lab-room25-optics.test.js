...
for(const options of [{order:'weight-first'},{order:'light-first',interruptLight:true}])test(`room25 optical shutters preserve the original friend: ${JSON.stringify(options)}`,async()=>{
 await g.selectLevel(24,false);const body=g.physics.cargoBody;let loaded=false,retrieved=false;
...
test('room25 an unweighted physical shutter blocks the actual source and leaves the first lift down',async()=>{
 ...
 await runV8Journey(g,{scenario:d=>{
  ...
  d.walk(17,6);d.walk(-20,6);d.walk(-20,-10);d.walk(-14,-10);d.aim(1,p['lower-relay'].getFrame().center);d.wait(5);
