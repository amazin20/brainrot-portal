...
test('fan is a sustained force, not a scripted launch, and no force survives switching it off',async()=>{
 ...
 l.state.enabled=true;l.update(1/120);assert.ok(l.playerAcceleration(l.state.airOrigin.clone().add(V(1,-1.1,0)),V()).x>19.5);
 assert.equal(l.playerAcceleration(V(-5,1,0),V()).lengthSq(),0);l.state.enabled=false;l.update(1/120);assert.equal(l.playerAcceleration(l.state.airOrigin.clone().add(V(1,-1.1,0)),V()).lengthSq(),0);
});
