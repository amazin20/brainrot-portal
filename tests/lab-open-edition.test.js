...
test('review edition accepts only its three actual replacements and keeps old links unchanged',()=>{
 assert.equal(readOpenEdition('?level=24').enabled,false);
 for(const n of [24,28,30])assert.deepEqual(readOpenEdition('?edition=open&level='+n),{enabled:true,levelIndex:n-1});
 for(const v of ['','0','NaN','99','1','24.5'])assert.equal(readOpenEdition('?edition=open&level='+v).levelIndex,23);
 assert.deepEqual(OPEN_ROOM_INDICES.map(nextOpenRoom),[27,29,23]);
});
