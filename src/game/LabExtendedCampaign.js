...
export function buildExtendedCampaign(game,index){
 ...
 if(index===5){
  ...
  const origin=V(fanBox.max.x+.04,2.1,7),airflow=new LabAirflowVisual(world.root,{radius:1.45});
  ...
  update=dt=>{state.previousFanPhase=state.fanPhase;const target=state.enabled?12:0,rate=state.enabled?3.5:2.7,old=state.fanSpeed;
   ...
   state.segments=state.enabled?tracePortalRay(game,origin,V(1,0,0),{medium:'air'}):[];
   airflow.step(dt,state.enabled?state.fanSpeed/12:0);airflow.setPath(state.segments,game.portals?.portals||[]);
  };
