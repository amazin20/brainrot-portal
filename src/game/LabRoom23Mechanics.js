...
export function buildRoom23Balance(k){
 ...
 const system={west,east,height:0,velocity:0,braked:false,loaded:false,coupling:'east',update(dt){
  const f=system.coupling==='west'?west.floor:east.floor,p=g.playerPosition,heldOnCoupledCar=Boolean(g.heldCube)&&g.playerGrounded&&Math.abs(p.y-f.y)<.2&&p.x>f.minX&&p.x<f.maxX&&p.z>f.minZ&&p.z<f.maxZ;
  // Carried freight still weighs down its car: lifting it in place is no cheat.
  system.loaded=(system.coupling==='west'?west.loaded():east.loaded())||heldOnCoupledCar;
 ...
 },reset(){system.height=system.velocity=0;system.braked=system.loaded=false;system.coupling='east';west.setY(0,0);east.setY(20,0);}};
