...
export async function runRoom20(d,{order='cargo-first',interruptPower=false,route='middle-cage'}={}){
 ...
 check(['middle-cage','early-return'].includes(route),'Unknown ascent route');
 ...
 if(route==='early-return'){
  walk(-21,-10);walk(-22.7,-10);
  for(let n=0;n<32;n++)walk(-22.7,-8.7+n*.6);
  check(game.playerPosition.y>15.9,'Western return-gallery stair was not reached');
  walk(-21,10.5);walk(-20,0);walk(-18,-15);walk(-12,-17);walk(-7,-17);walk(-5,-14.3);
  check(s.optical.loaded&&s.optical.receivers[1]&&!s['second-cage'].powered,
   'Original cargo must remain on the live reflector while player scouts high');
  mark('player reaches upper gallery before moving cargo from original mirror');
  walk(-7,-14.3);aim(1,p['light-output'].getFrame().center.clone().add({x:0,y:0,z:2}));wait(2);
  until(()=>s['return-cage'].position.y<16.01,12,'Return car did not descend when beam moved');
  mark('real ray interruption lowers the loaded return car');
  walk(-2,-17);walk(-2,-12);walk(-3.2,-10.2);walk(-7,-9.7);walk(-9,-9.4);
  aim(1,p['light-output'].getFrame().center);wait(2);
  check(s.optical.receivers[1],'Loaded reflector did not repower return car');
  until(()=>game.playerPosition.y>21.98,12,'Early return ascent');
  mark('player rides return car while original cargo stays on ground mirror');
  walk(-7,-9.7);walk(-3.2,-10.2);walk(-2,-12);walk(-2,-17);walk(-15.2,-18);
  check(game.playerPosition.y>21.9,'Return car did not connect to final permanent deck');
 }else{
