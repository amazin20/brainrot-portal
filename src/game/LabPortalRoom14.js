...
export function buildRoom14(game,index=13){
 ...
 // An exposed receiver on the island gives the first crossing a second
 // solution: leave the free companion at the entry, cross on light alone,
 // then route it from the entry floor into this solid landing.
 w.box([3.05,9.55,0],[.2,4.3,6],w.materials.wall);
 // A raised horizontal sight slot in the left wall lets the player at the
 // upper stair aim at the entry. The full-height outer pinch remains intact.
 w.box([-3.15,8.65,-8.25],[.3,2.5,7.5],w.materials.wall);
 w.box([-3.15,18.175,-8.25],[.3,13.65,7.5],w.materials.wall);
 w.box([-3.15,8.125,-3.75],[.3,1.45,1.5],w.materials.wall);
 w.box([-3.15,17.625,-3.75],[.3,14.75,1.5],w.materials.wall);
 w.box([.15,16.2,-7.5],[.3,17.6,9],w.materials.wall);
 // Full-height outer fold keeps the player-and-cube pinch unchanged. The
 // player alone leans left on the stair and sees the entry via the inner slot.
 ...
 level.puzzleGeometry={footprint:1760,safeFloor:0,goalHeight:13.4,portalRoles:{access:'return access to the light landing','weave-west':'first bridge across the lower crossing','light-source':'capture the physical light sheet','weave-north':'perpendicular upper bridge across the deep north','return-floor':'turn the sheet into a wall and return below','island-floor':'required independent cargo delivery from the stable island','entry-dispatch':'the original friend waits at the start until the player reaches stable ground','island-receiver':'receives the free companion after the player alone crosses the first light bridge'},deductions:['one portal pair serves both support and transport','the narrow fold requires independent cargo delivery','restore the emitter from a lower sight and reuse the upper route','the first bridge can carry player and companion by different routes'],orders:['cargo-first','remote-companion']};
 return level;
}
