...
export async function runRoom24(d,{order='charge-first',interruptFeed=false,storageDelay=0}={}){
 ...
 walk(-14,-14.5);walk(-12.6,-14.5);game.interact();check(!s.pneumatic.liftValve,'Cylinder valve did not isolate');
 ...
 walk(-3,-12);walk(8,-12);walk(8,-9.8);walk(8.2,-6.3);game.interact();wait(.4);walk(9.15,-6.6);game.interact();check(s.pneumatic.bridgeValve,'Crossing valve did not open');walk(game.cargo.position.x-.8,game.cargo.position.z);pickup();mark('finite reserve carries both travellers');
 until(()=>s['pressure-ferry'].progress>.997,30,'Reservoir did not carry the loaded crossing');
 walk(8,14.4);walk(17,17);until(()=>game.state==='won',3,'Both travellers missed the pressure dock');
}
