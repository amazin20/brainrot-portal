...
export async function runRoom24(d,{route='carry-through',recovery=false}={}){
 ...
 if(recovery){game.clearPortals();wait(.2);walk(-7,-8);d.aim(1,p['revolving-door'].getFrame().center);mark('erased pair restored from the permanent garden');}
 walk(-20,-8);walk(-20,-18);check(game.playerPosition.y>9.9,'Garden reverse perch missed');d.aim(0,p['pavilion-receiver'].getFrame().center);mark('folded garden stair reveals the yellow pavilion');walk(-20,-7);walk(game.cargo.position.x-.9,game.cargo.position.z);pickup();enter(p['revolving-door']);walk(16,-18);until(()=>game.state==='won',4,'Garden joint arrival missed');mark('original companion joins the final garden');
}
