...
export const ROOM13_SPEC={id:'optical-paradox',title:'Оптический парадокс',concept:'Один луч и живой противовес меняют доступные стороны оптического узла',description:'Свету и другу нужен один и тот же проход. Найди, когда его можно перестроить.',hints:['Путь луча зависит от того, кто стоит на зеркальной опоре.','Порталы нужны и свету, и вам. Верхний обход позволяет сохранить достигнутую высоту.','Нагруженное зеркало направляет свет к южной кабине. Можно сперва подняться северной кабиной со спутником, затем с постоянной галереи отправить его на зеркало через второй адрес.'],accent:0xf1c981,assets:[1,2,11,23,24]};
export function buildRoom13(game,index=12){
 ...
 // The 0.9 m horizontal sight band passes a beam to the lower mirror while
 // its overhead beam blocks the standing 2.4 m traveller from crossing.
 w.box([3,12.05,-12.8],[18,.1,.18],w.materials.wall);
 w.box([3,13.2,-12.8],[18,.4,.18],w.materials.wall);
 w.box([3,12.7,12.8],[18,1.4,.18],w.materials.wall);
 ...
 // A distinct address at observation height receives a companion separately
 // after both travellers rode the otherwise unweighted northern cage.
 k.panel('north-dispatch',[-12.85,14,-14.5],[1,0,0],4.6,4.0);
 k.panel('light-intake',[19.5,8,-12],[-1,0,0],5.6,4.6);
 k.panel('light-output',[-17.5,8,0],[1,0,0],5.6,4.6);
 const pad=k.pad('mirror-cradle',[-10,6,0],5.2,5.2);
 ...
 level.puzzleGeometry={footprint:40*38,orders:['cargo-first','north-with-companion'],portalRoles:{'access-low':'leave and return to folded optical shelf','access-high':'reach the mirror and move the original load','north-dispatch':'dispatch original companion separately from the permanent northern observation gallery','light-intake':'capture the physical source ray','light-output':'redirect the source into the weighted mirror','mirror-cradle':'carry load, change reflection, later free the same friend','receiving-return':'recover the friend after releasing the optical route'},deductions:['portals are shared between light and bodies','weight changes the actual reflection angle','retain height before replacing the powered portal pair','an unweighted lift can raise the companion before it becomes the mirror load']};return level;
}
