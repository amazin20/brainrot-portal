...
export async function runRoom30(d,{east=false,recovery=false,inspectFirst=false}={}){
 ...
 const prepare=()=>{walk(-21,13);walk(-25,13);walk(-25,-32);approach(-22.6,-32);aim(0,p['first-well'].getFrame().center);aim(1,p[east?'east-arc':'north-arc'].getFrame().center);};
 const firstDive=()=>{
 ...
 walk(42,30);walk(40,45);approach(43.7,45);aim(0,p['second-well'].getFrame().center);aim(1,p['sunward-outlet'].getFrame().center);
 mark('room30 final flight prepared');walk(42,30);walk(60,30);const before=game.teleportCount;
 for(let n=0;n<240&&game.playerGrounded;n++){worldMove(0,1);game.input.keys.add('ShiftLeft');frame();}
