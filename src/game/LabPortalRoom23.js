...
export function buildRoom23(game,index=22){
 ...
 k.control('balance-coupling',[-13.3,0,5.6],()=>{balance.coupling=balance.coupling==='west'?'east':'west';},'E — связать противовес с этой кабиной');
 ...
 level.puzzleGeometry={footprint:46*45,occupiedHeights:[0,10,20],orders:['receiver-first','coupled-west-first'],noProgressFlags:true,portalRoles:{'west-load-car':'floor aperture below the original load on the near car','east-load-car':'the same distant weighted carriage later carries the player upward','freight-throat':'cargo-only delivery to the distant high car','middle-pocket':'permanent storage preserves cargo while the counterweight reverses','high-return':'final retrieval from the reverse high gallery'},deductions:['opposite car motion exchanges height instead of creating an unlocked stage','the physical coupling chooses which original load can drive the first ascent','send the original load ahead or ride with it in the western carriage','a physical brake preserves both occupied heights','lifting the load inside the coupled cabin does not unload the transmission','the fixed middle pocket frees the same high car for its return','retrieve the original load through its resting floor from the newly reached viewpoint']};
 return level;
}
