...
export function buildRoom15(game,index=14){
 ...
 level.puzzleGeometry={footprint:42*38,cargoThroatHeight:1.55,goalHeight:14,portalRoles:{intake:'routes both push and pull from the real emitter',freight:'extracts the original friend through a low throat',lift:'raises player and friend into the shared observation pocket',perch:'holds the friend for retrieval after the crossing',crossing:'carries the player transversely above the dividing spine',receiver:'retrieves the friend from the upper perch'},orders:['separate-and-retrieve','carry-together-through-crossing'],deductions:['reverse a routed field to extract cargo','leave a vertical field at the useful height','reroute a field using a new viewpoint','either separate travellers and reunite through a floor pair or steer the joint airborne crossing']};
 return level;
}
