...
export const CAMPAIGN=Object.freeze([...INTRODUCTORY_CAMPAIGN,...EXTENDED_CAMPAIGN.slice(0,3),...WORKSHOP_CAMPAIGN.map((room,i)=>i===2?{...room,assets:room.assets.filter(id=>id!==39),accent:0x83cfc7,description:'Направь воздух от вентилятора к приводу двери.',hints:['Круглый вентилятор слева создаёт поток. Привод с решёткой дальше по залу принимает воздух передней стороной.','Поток сам по себе не открывает дверь: второй пульт подключает привод. Кабель показывает, к какой двери он относится.','Соедини правую стену напротив вентилятора с другим участком правой стены напротив привода. Включи вентилятор и подключи привод вторым пультом. Забери друга и пройди в открывшуюся дверь.']}:room)]);
