...
export const ROOM24_SPEC={id:'garden-of-turning-doors',title:'Сад поворотных дверей',concept:'Портал помнит дверь, а не направление; один поворот открывает другой сад',description:'У каждого сада есть лицевая и обратная сторона. Посмотри, что меняется вместе с дверью и что остаётся на месте.',accent:0xffc75b,assets:[1,2,11,22,23,24],hints:['Одна керамическая дверь смотрит в два разных двора. Портал поворачивается вместе с ней.','Грузовая клумба управляет поворотом. В западном саду есть тормоз, удерживающий настоящее положение двери.','Подними друга через его опору. Северная лестница меняет ракурс: за стеной видна обратная сторона жёлтого павильона.']};
export function buildRoom24(game,index=23){
 ...
 for(let i=0;i<16;i++){const z=-8-i*.5;deck('Garden folded stair',-23,-18,z-.5,z,6+(i+1)*.25);block([-20.5,6+(i+.5)*.25,z-.465],[5,.27,.07],mint);}
 deck('High reverse pergola',-24,-16,-23,-16,10);
 ...
 k.panel('balcony-entry',[6.6,8.3,3.5],[-1,0,0],5,5);
 k.panel('garden-entry',[18,2.3,23.6],[0,0,-1],6.4,5);
 ...
 const positions=[[11,0,14],[20,0,8],[-20,0,17],[-21,6,-1],[-22,10,-20],[20,15,-14]];
 for(const [x,y,z] of positions){block([x,y+.6,z],[1.8,1.2,1.8],pot);const m=new THREE.Mesh(new THREE.IcosahedronGeometry(1.45,0),leaf);m.userData.keepMaterial=true;m.position.set(x,y+2.2,z);w.root.add(m);}
 ...
 level.spawnView={yaw:.4,pitch:-.1};level.puzzleGeometry={footprint:2304,occupiedHeights:[0,6,10,15],goalHeight:15,orders:['door-first','weight-first'],noProgressFlags:true,portalRoles:{'garden-counterweight':'the original weight becomes its own return aperture','garden-entry':'stable entrance below the rotating door','revolving-door':'one persistent portal changes the courtyard it faces','pavilion-receiver':'a high reverse view into the gold pavilion'},deductions:['a portal retains the moving architectural face','a weight turns the door towards a separate courtyard','the courtyard brake preserves orientation after the weight leaves','a floor aperture recovers that same weight','the folded stair exposes the pavilion from its reverse side']};
 return level;
}
