...
export async function runRoom17(d,{order='cargo-first',interruptBrake=false,route='canonical'}={}){
 const {game,level,walk,wait,aim,look,until,pickup,enter,mark,worldMove,frame,stop}=d,p=level.panels,m=level.state.address;
 check(['canonical','cargo-first-momentum'].includes(route),'Unknown moving-address route');
 ...
 until(()=>m.progress<.001,6,'Return stroke completion');
 if(route==='cargo-first-momentum'){
  // Lower the same travelling portal so a loose companion at deck height can
  // enter it. Release while walking: its own inherited velocity crosses the
  // threshold before the player follows from the original berth.
  aim(1,p.address.getFrame().center.clone().add({x:0,y:-.2,z:0}));
  walk(game.cargo.position.x-1.05,game.cargo.position.z);pickup();walk(-14,8.1);
  for(let n=0;n<100&&game.playerPosition.z>7.4;n++){worldMove(0,-1);frame();}
  stop();check(game.playerPosition.z<=7.4,'Could not line up the independent companion delivery');
  const cargoBefore=game.physics.portalTransports,playerBefore=game.teleportCount;
  game.interact();check(!game.heldCube,'Companion did not leave the hands');
  until(()=>game.physics.portalTransports>cargoBefore,2,'Loose companion missed the lower carriage aperture');
  check(game.teleportCount===playerBefore,'Player crossed before the loose companion');
  mark('loose companion reached the upper receiver before the player');
  enter(p.address);
 }else{
  walk(game.cargo.position.x-1.05,game.cargo.position.z);pickup();enter(p.address);
 }
 walk(-16,-20.4);until(()=>game.state==='won',4,'Moving address reunion');
}
