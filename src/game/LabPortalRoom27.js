...
export function buildRoom27(game,index=26){
 ...
 const launch=k.panel('sky-well',[4,.025,7],[0,1,0],6,6);
 ...
 deck('Upper belt inner inspection quay',10,13,-19,-1,8);
 belts.belt('Turquoise upper return belt',15.3,-19,9,8.08,{width:4.6,speed:28});
 k.panel('upper-intake',[15.3,9.88,-19.2],[0,0,1],5.6,4.6);
 for(const x of [12.25,18.35])block([x,6.5,-9.6],[.3,13,19.6]);
 block([15.3,13,-9.6],[6.4,.3,19.6]);
 ...
 block([6.5,15.75,0],[7,.35,7]);
 block([6.5,20.5,3.7],[7,17,.35]);
 block([5.5,20.5,-3.7],[5,17,.35]);
 block([9,8,-3.7],[2,16,.35]);
 block([9,24.5,-3.7],[2,9,.35]);
 ...
 const level=k.finish([-20,0,15.8],[-17,.55,19.8],[-18,8,0],{workshop:k,spec:ROOM27_SPEC,portalPuzzle:true,playerAcceleration:(p,v)=>belts.acceleration(p,v,{grounded:game.playerGrounded})});
 level.spawnView={yaw:-.3,pitch:-.22};
