...
export async function runRoom27(d,{inspectEastFirst=false,gravityReturn=false,recoverFirst=false}={}){
 ...
 if(inspectEastFirst){walk(4,19);walk(4,10);aim(1,p['sky-well'].getFrame().center);walk(10,12);walk(10,19);walk(-19,19);mark('the eastern loop reveals the sky well before the motor');}
 walk(-19,16);game.interact();wait(.3);check(!s.reversed,'Lower reversing drum did not reverse the belt');
 walk(-7,16);aim(0,p['ground-intake'].getFrame().center);walk(4,10);aim(1,p['sky-well'].getFrame().center);
 ...
 walk(8,-23.5);walk(8,-7);walk(6,-7);walk(6,0);
 if(gravityReturn){look(game.playerPosition.clone().add({x:2,y:1,z:0}));game.interact();wait(1);}
 aim(1,p['reverse-outlet'].getFrame().center);mark('the upper return reveals the reverse side of the high outlet');
 if(gravityReturn){
  walk(6,1.2);walk(3.2,1.2);aim(0,p['gravity-post'].getFrame().center);walk(6,1.2);walk(game.cargo.position.x-.5,game.cargo.position.z);pickup();
  walk(7,-7);walk(7,-10);walk(1,-10);
 ...
 walk(8,-7);walk(8,-23.5);walk(21,-23.5);walk(21,-8);walk(21,11);
 walk(10,11);walk(10,-14);aim(0,p['upper-intake'].getFrame().center);walk(10,7);walk(game.cargo.position.x+1,game.cargo.position.z);pickup();walk(10,11);walk(15.3,11);
 const high=game.teleportCount;
 mark('ready for the upper conveyor crossing');
 for(let n=0;n<400&&game.teleportCount===high;n++){worldMove(0,-1);frame();}stop();check(game.teleportCount>high,'Upper belt did not enter the readdressed pair');
